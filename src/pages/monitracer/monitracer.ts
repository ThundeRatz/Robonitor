...
export class MonitracerPage {
  ...
  save(): void {
    ...
    if (this.constants.value['speed0'] != "") {
      let speed0: number = this.constants.value['speed0'];
      this.send([0xFF, CMD_LSPD, 0, speed0 >> 8, speed0 & 0xFF, 0xFE]);
      this.send([0xFF, CMD_RSPD, 0, speed0 >> 8, speed0 & 0xFF, 0xFE]);
      console.log(speed0);
    }

    if (this.constants.value['speed1'] != "") {
      let speed1: number = this.constants.value['speed1'];
      this.send([0xFF, CMD_LSPD, 1, speed1 >> 8, speed1 & 0xFF, 0xFE]);
      this.send([0xFF, CMD_RSPD, 1, speed1 >> 8, speed1 & 0xFF, 0xFE]);
      console.log(speed1);
    }

    if (this.constants.value['speed2'] != "") {
      let speed2: number = this.constants.value['speed2'];
      this.send([0xFF, CMD_LSPD, 2, speed2 >> 8, speed2 & 0xFF, 0xFE]);
      this.send([0xFF, CMD_RSPD, 2, speed2 >> 8, speed2 & 0xFF, 0xFE]);
      console.log(speed2);
    }
  }
